...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_mask):
        ...
        a_output = self.attention(q=x, k=x, v=x, mask=src_mask)
...
class ExtendedEncoderLayer(EncoderLayer):
    ...
    def forward(self, x, src_mask, relative_ids):
        # Compute self attention
        a_output = self.attention(q=x, k=x, v=x, relative_ids=relative_ids, mask=src_mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_output, src_mask, trg_mask):
        # Compute self attention
        a_output = self.self_attention(q=x, k=x, v=x, mask=trg_mask)
        ...
        if enc_output is not None:
            ...
            a_output = self.cross_attention(q=x, k=enc_output, v=enc_output, mask=src_mask)
...
class ExtendedDecoderLayer(DecoderLayer):
    ...
    def forward(self, x, enc_output, src_mask, trg_mask, relative_ids):
        # Compute self attention
        a_output = self.self_attention(q=x, k=x, v=x, relative_ids=relative_ids, mask=trg_mask)
        ...
        if enc_output is not None:
            ...
            if self.check == "rel-e" or self.check == "rel-b" or self.check == "rel-eb":
                ...
                a_output = self.cross_attention(q=x, k=enc_output, v=enc_output, relative_ids=relative_ids, mask=src_mask)
...
class ExtendedEncoder(nn.Module):
    ...
    def forward(self, x, src_mask, relative_ids):
        for layer in self.layers:
            x = layer(x, src_mask, relative_ids)
...
class ExtendedDecoder(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask, relative_ids):
        for layer in self.layers:
            trg = layer(trg, enc_src, trg_mask, src_mask, relative_ids)
...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, trg_vocab_size, d, h, l, f, max_seq_length, dropout):
        super(Transformer, self).__init__()
        self.encoder_embedding = nn.Embedding(src_vocab_size, d)
        self.decoder_embedding = nn.Embedding(trg_vocab_size, d)
        self.positional_encoding = Encoding.AbsolutePositionalEncoding(d, max_seq_length)
    ...
    def forward(self, src, trg):
        ...
        tgt_embedded = self.dropout(self.positional_encoding(self.decoder_embedding(trg)))
...
class ExtendedTransformer1(Transformer):
    def __init__(self, src_vocab_size, trg_vocab_size, d, h, l, f, max_seq_length, dropout, attention="rel-eb"):
        super(ExtendedTransformer1, self).__init__(src_vocab_size, trg_vocab_size, d, h, l ,f, max_seq_length, dropout)
        self.encoder = ExtendedEncoder(l, d, h, f, dropout, attention)
        self.decoder = ExtendedDecoder(l, d, h, f, dropout, attention)
    
    def forward(self, src, trg, relative_ids):
        src_mask, trg_mask = self.generate_mask(src, trg)
        src_embedded = self.dropout(self.positional_encoding(self.encoder_embedding(src)))
        tgt_embedded = self.dropout(self.positional_encoding(self.decoder_embedding(trg)))

        enc_output = src_embedded
        enc_output = self.encoder(enc_output, src_mask, relative_ids)

        dec_output = tgt_embedded
        dec_output = self.decoder(dec_output, enc_output, trg_mask, src_mask, relative_ids)
        output = torch.softmax(self.fc(dec_output), dim=-1)
...
class CopyDecoder(nn.Module):
    def __init__(self, trg_vocab_size, d_model, num_heads, dropout):
        ...
        self.fco = nn.Linear(trg_vocab_size, d_model)
        self.fcw = nn.Linear(trg_vocab_size, 1)
    ...
    def forward(self, src_vocab_size, dec_output, enc_output, src, p1):
        ...
        a_output = self.attention(q=copy_query, k=enc_output, v=src_one_hot) # Nel codice non usano una mask
...
class ExtendedTransformer2(ExtendedTransformer1):
    def __init__(self, src_vocab_size, trg_vocab_size, d, h, l, f, max_seq_length, dropout, attention="rel-eb"):
        super(ExtendedTransformer2, self).__init__(src_vocab_size, trg_vocab_size, d, h, l, f, max_seq_length, dropout, attention)
        self.copy_decoder = CopyDecoder(trg_vocab_size, d, h, dropout)
    
    def forward(self, src, trg, relative_ids, src_vocab_size):
        src_mask, trg_mask = self.generate_mask(src, trg)
        src_embedded = self.dropout(self.positional_encoding(self.encoder_embedding(src)))
...
class ExtendedTransformer4(ExtendedTransformer2):
    def __init__(self, src_vocab_size, trg_vocab_size, d, h, l, f, max_seq_length, dropout, attention="rel-eb", shared_weights=False):
        super(ExtendedTransformer4, self).__init__(src_vocab_size, trg_vocab_size, d, h, l, f, max_seq_length, dropout, attention)
